...
def main(args):
    ...
    result_df = pd.DataFrame(
        columns=['Data Name', 'Targets', 'Attributes', 'Method', 'Score', '# of target words', '# of attribute words'])
    ...
    with open(args.weat_path) as f:
        ...
        for data_name, data_dict in weat_dict.items():
            if data_dict['method'] == 'wefat':
                #print('{}: WEFAT is not implemented'.format(data_name))
                #continue
                ...
                num_target = len(W)
                num_attr = len(A)
                SC_score = weat.weat_association(W, A, B)
                ##SC_p_value = weat.wefat_p_value(W, A, B, parametric= True, num=1000)


            elif data_dict['method'] == 'weat':
                print('{}: WEAT is implemented'.format(data_name))
                ...
                print(num_target)
                num_attr = len(A)
                P_vec = weat.get_p_value(X,Y,A,B, "normal" ,100)
                """
                score = weat.weat_score(X, Y, A, B)
                p_value = weat.weat_p_value(1000, X, Y, A, B, parametric=True)
                """
            else:
